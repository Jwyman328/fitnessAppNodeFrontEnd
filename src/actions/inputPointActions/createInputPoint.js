 ...
 * @param {object} inputPointFormState -- current state of pointInput activity submission attempt.
 * @param {*} dispatch  -- dispatcher that sends an action object to the pointInputReducer.
 */
async function CreateInputPoint(inputPointFormState, dispatch, token) {
  const {
    date,
    sleepHours,
    water100oz,
  ...
  const createInputData = {
    ...
    water100Oz: water100oz,
    cleanEating: cleanEating,
    workoutIntensity: workoutIntenisty,
    workoutTimeLength: workoutTime,
    steps: steps
  };
